(() => {
  ...
  async function uploadFile(file) {
    ...
    try {
      const res = await fetch("/screensaver/upload", { method: "POST", body });
      ...
      showPreview(data.preview_url || "/screensaver/preview");
  ...
  async function pushToKindle() {
    ...
    try {
      const res = await fetch("/screensaver/push", { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
        setStatus({
          converted: true,
